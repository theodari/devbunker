    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (oauthServers.length === 0) {
          ...
          prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in devbunker.json (or opencode.json):")
          prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  ...
  const candidates = [
    path.join(baseDir, "devbunker.json"), path.join(baseDir, "devbunker.jsonc"),
    path.join(baseDir, "opencode.json"), path.join(baseDir, "opencode.jsonc"),
  ]

  if (!global) {
    candidates.push(
      path.join(baseDir, ".opencode", "devbunker.json"), path.join(baseDir, ".opencode", "devbunker.jsonc"),
  ...
  // Default to devbunker.json if none exist
  return candidates[0]
}
