...
export const RunCommand = cmd({
  command: "run [message..]",
  describe: "run devbunker with a message",
  builder: (yargs: Argv) => {
    return yargs
      .positional("message", {
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running devbunker server (e.g., http://localhost:4096)",
      })
      .option("dir", {
        type: "string",
  ...
  handler: async (args) => {
    ...
    await bootstrap(process.cwd(), async () => {
      ...
      const sdk = createOpencodeClient({ baseUrl: "http://devbunker.internal", fetch: fetchFn })
      await execute(sdk)
    })
  },
})
